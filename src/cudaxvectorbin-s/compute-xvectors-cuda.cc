...
CachingOptimizingCompiler* Nnet3XvectorInit(
		ParseOptions& po,
	...
	CachingOptimizingCompiler* compiler = new CachingOptimizingCompiler(*nnet, opts->optimize_config, compiler_config);
...
bool Nnet3XvectorCompute(const Matrix<BaseFloat> &features, std::string &utt, Vector<BaseFloat>* xvector_avg, const Nnet& nnet,
		int32 chunk_size, int32 min_chunk_size, bool pad_input,  CachingOptimizingCompiler *compiler) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    CachingOptimizingCompiler* compiler = Nnet3XvectorInit(po, &nnet3_opts, &nnet, &cached_compiler_out,
    		&chunk_size,  &min_chunk_size, &pad_input);
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      if (ComputeMFCC(&wave_data, min_duration, channel, mfcc, &features, utt) == false){
    	  num_fail++;
    	  continue;
      }
      //VAD
      ...
      //apply cmvn sliding
      Matrix<BaseFloat> cmvn_feat(features.NumRows(), features.NumCols(), kUndefined);
      ...
      int32 dim = 0;
	   ...
	   if (Nnet3XvectorCompute(voiced_features, utt, &xvector, nnet,
			   chunk_size, min_chunk_size, pad_input, compiler) == false){
		   num_fail++;
		   continue;
	   }
